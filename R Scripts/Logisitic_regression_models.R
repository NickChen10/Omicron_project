...
metadata_clean<- metadata %>% select(sample_id,collection_date,mpi, age,gender,city_gsheet,ShortLin,vax_status_revised,vax_status) %>% 
  rename(town = city_gsheet, variant = ShortLin)
old_dataset_clean<- old_dataset %>% select(sample_id,collection_date,mpi, age,gender,city,lineage_name,vax_status_revised,vax_status) %>% 
